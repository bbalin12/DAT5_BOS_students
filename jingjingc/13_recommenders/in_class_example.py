# -*- coding: utf-8 -*-
"""
Created on Thu Mar  5 20:10:12 2015
...
df = pd.read_csv("/Users/jchen/Documents/beer_reviews/beer_reviews.csv")
df.head()
...
class BeerRecommender(YhatModel):
    ...
    def execute(self, data):
        my_beer = data.get("beers")
        suggested_beers = similar_beers(my_beer)
        result = []
        for beer in suggested_beers:
            result.append({"beer": beer})
        return result
        
BeerRecommender().execute({"my_beers": ["Coors Light"]})

yh = Yhat("jchen027@gmail.com", "07d6ecaa751895410a21b0c24b7a5a51", "http://cloud.yhathq.com/")
yh.deploy("BeerRecommender", BeerRecommender, globals())
yh.predict("BeerRecommender",{"my_beer": ["Coors Light"]})